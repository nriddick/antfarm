...
measured first with unchanged workload and allocator settings. The caller
choices of mimalloc, wave batching, and consumer placement are measured
separately. DRuntime Fibers remain the project's Fiber execution model.

The retained benchmark supports the existing C-runtime default (`crt`), the
optional mimalloc adapter (`mimalloc`), and its matching-alignment diagnostic
(`mimalloc64`). Experimental pool, arena, and alternative-malloc policies have
been removed. For the earlier removal of quadratic actor-creation, ready-queue,
and Fiber lifecycle scans, see [README.md](README.md); for allocator setup and
the first mimalloc comparison, see [MIMALLOC.md](MIMALLOC.md).
...
Wave mode accepts publication batches larger than 256. Passing the whole
cohort lets the existing Farm API choose physical table sizes. The autonomous
runtime's public flush limit remains 256. Both modes retain every actor
lifetime operation and correctness check. Each measured cycle allocates and
frees every actor's state; stable runtime storage survives across cycles.
...
Replace the consumer argument `0` with `6` to add six polling consumers;
the controlling thread still owns all actor creation and reclamation.
...
least two measured seconds per run. The same benchmark source at the time
was built against `deb987e` and `0425226` (the retirement change). The table
uses three runs per configuration in reversed/rotated order, with the CPU placements above.
...
ThreadSanitizer and the real mimalloc `MI_DEBUG=FULL` lane pass. The pinned
timed comparisons pass exact per-cycle execution/reclamation checks. Large-wave and affinity smoke checks also pass in LDC release and
DMD debug builds. Invalid CPU-list length and unavailable CPUs fail explicitly.
...
Their additional Pcount fetch-add therefore performs no arbitration. It is
replaced with a raw atomic store recording the unique claim. Callbacks still observe
...
The same unchanged churn harness compares `0425226` with `cf768cf`, using
...
complete lifetimes/second; the timed runs pass the lifecycle checks.
...
### Larger wave offers and consumer placement

The same `cf768cf` comparison also measures the caller offering a whole wave
at once, with mimalloc and the same placements. These are direct measurements,
not a sum of percentage gains from earlier runs.

| Execution | Autonomous, batch 256 | Wave, batch 256 | Wave, whole-cohort offer |
| --- | ---: | ---: | ---: |
| Same thread, CPU 2 | 18.247 | 22.346 | 22.713 |
| Six consumers, controller CPU 6 | 14.992 | 15.450 | 15.940 |

A separate three-run measurement gives the controller CPU 6 and one consumer
CPU 1, on different physical cores. With mimalloc, autonomous batch 256 reaches
**20.643 million complete lifetimes/s**, and whole-wave offers reach
**25.192 million complete lifetimes/s**. All six runs pass the lifecycle checks.

```sh
./perftest/actor_churn_mimalloc actor 16384 2 1 256 5 mimalloc 6,1
./perftest/actor_churn_mimalloc wave 16384 2 1 16384 5 mimalloc 6,1
```

For this tiny-callback workload, the separate consumer helps, while six
continuously polling consumers cost more than they contribute. Creation and
reclamation remain serial on the controller. The six-consumer placement also
puts a poller on the controller's SMT sibling; these measurements do not
isolate worker count from topology. Applications with substantial callback
work should measure their own worker counts and placements.

All tables record the indicated historical revisions and harness. Removing
the experimental allocator modes changes the benchmark executable's layout;
these are reference measurements, not fresh timings of the cleaned harness.
